...
- **Fixed**: SPDX license ID list is now inlined as a TypeScript constant.
  Removes the runtime JSON import that crashed under Node 22 / 24 strict
  ESM with `[ERR_IMPORT_ATTRIBUTE_MISSING]`. The list is generated by
  `packages/validator/scripts/gen-spdx-ids.ts` and committed at
  `src/spdx-ids.ts`; regenerate with `bun run scripts/gen-spdx-ids.ts`.
- **Added**: `engines: { "node": ">=20" }` to `package.json`.
- **Internal**: consumer integration test
  (`scripts/__tests__/consumer-integration.test.ts`) installs the packed
  tarball into a fresh ESM project and runs `validateSkill` via plain
  `node` — the exact path real consumers walk.
